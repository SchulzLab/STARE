...
[-n Normalization approach (KR/VC/VC_SQRT, Default KR)]\n"
...
if [[ $chromosomes == *"-"* ]]; then
  ...
  for i in $(seq "${ADDR[0]}" "${ADDR[1]}"); do
    java -jar ${jar_file} dump observed "${normalization}" "${hic_file}" chr${i} chr${i} BP ${bin_size} ${out_folder}/chr${i}_"${normazliation}"_Contacts.txt;
    gzip ${out_folder}/chr${i}_"${normalization}"_Contacts.txt;
  done
  # Manually check if any gonosome was selected in addition to the range.
  if [[ $chromosomes == *"X"* ]]; then
    java -jar ${jar_file} dump observed "${normalization}" ${hic_file} chrX chrX BP ${bin_size} ${out_folder}/chrX_"${normalization}"_Contacts.txt;
    gzip ${out_folder}/chrX_"${normalization}"_Contacts.txt;
  fi
  if [[ $chromosomes == *"Y"* ]]; then
    java -jar ${jar_file} dump observed "${normalization}" ${hic_file} chrY chrY BP ${bin_size} ${out_folder}/chrY_"${normalization}"_Contacts.txt;
    gzip ${out_folder}/chrY_"${normalization}"_Contacts.txt;
  fi
elif [[ $chromosomes == *","* ]]; then
    IFS=',' read -ra csv <<< "$chromosomes"
    for i in "${csv[@]}"; do
    java -jar ${jar_file} dump observed "${normalization}" ${hic_file} chr${i} chr${i} BP ${bin_size} ${out_folder}/chr${i}_"${normalization}"_Contacts.txt;
    gzip ${out_folder}/chr${i}_"${normalization}"_Contacts.txt;
    done
else  # If only one individual chromosome was queried.
    java -jar ${jar_file} dump observed "${normalization}" "${hic_file}" chr${chromosomes} chr${chromosomes} BP ${bin_size} ${out_folder}/chr${chromosomes}_"${normalization}"_Contacts.txt;
    gzip ${out_folder}/chr${chromosomes}_"${normalization}"_Contacts.txt;
fi
