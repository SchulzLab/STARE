...
int main(int argc, char **argv) {
    ...
    int arg;
    while ((arg = getopt_long_only(argc, argv, "b:a:u:n:w:k:t:c:f:o:x:d:h:q:m:", long_options, nullptr)) != -1) {
        switch (arg) {
            case 'b':
                b_peakfile = optarg;
                break;
    ...
    for (const string& p: {b_peakfile, a_promoterfile, f_contactfolder, o_prefix}) {
        if (p.empty()) {
    ...
    int gene_counter;
    unordered_map <int, vector<string>> promoter_map;  // For ID, name, chr, TSS.
        ...
        if (fgets(annot_buffer, annot_len, Read_Gene_Annotation) != nullptr) {
            ...
            if ((row.size() > 1) and (row.rfind('#', 0) != 0)) {
                ...
                if (columns[2] == "gene") {
                    ...
                    auto prom_iter = promoter_map.find(gene_counter);
                    if (prom_iter != promoter_map.end()) {
                        if (((stoi(prom_iter->second[3]) > stoi(gene_start)) and (strand == "+") and
                             (prom_iter->second[2] == chr)) or
                            ((stoi(prom_iter->second[3]) < stoi(gene_start)) and (strand == "-") and
                             (prom_iter->second[2] == chr))) {
                            promoter_map[gene_counter][3] = gene_start;
                        }
                    } else {
                        gene_id_map[gene_counter] = gene_id;
                        promoter_map[gene_counter] = vector < string > {gene_id, gene_name, chr, gene_start};
...
#pragma omp critical(peek_open)
        ...
        boost::numeric::ublas::mapped_matrix<double> contact_matrix(max_bin + 1 - min_bin, max_bin + 1 - min_bin);
        ...
        while (!feof(hic_stream)) {
            if (fgets(hic_buffer, 256, hic_stream) != nullptr) {
                ...
                if (!isnan(contact) and contact > 0) {  // 0 Shouldn't appear if preprocessing done correctly.
                    ...
                    int second_bin = stoi(hic_row.substr(first_tab + 1, hic_row.find(delimiter, second_tab))) / bin_size;
                    int bin1 = first_bin;
                    int bin2 = second_bin;
...
#pragma omp critical(hic_close)
        ...
        unordered_map <int, vector<double>> gene_contact_map; // Same order as peak_map, but lists the contacts.
            ...
            if (gene_peak_map.count(chr_gene) == 1) {
                ...
                for (auto peak = gene_peak_map[chr_gene].begin(); peak != gene_peak_map[chr_gene].end();) {
                    ...
                    if (distance <= gene_windowsize) {
                        ...
                        ++peak;
                    } else {  // Remove the peaks that are only in the enh-window, only needed them for the contact_sum.
        ...
        for (const int& chr_gene : chr_gene_map[chr]) {
            ...
            if (u_genefile.length() != 0 and u_genefile != "0") {
                if ((filter_genes.count(promoter_map[chr_gene][0]) == 0) and (filter_genes.count(promoter_map[chr_gene][1]) == 0)) {
                    continue;
                }
            }
            ...
            vector <vector<double>> candidate_activities(num_candidates, vector<double>(3 * (col_num)));
            ...
            for (const int& peak_id : gene_iter->second) {
                ...
                candidate_contacts[peak_count_helper] = current_contact;
                ...
                for (int a_col = 0; a_col < col_num; a_col++) {
                    ...
                    candidate_activities[peak_count_helper][a_col * 3] = current_activity;
                    if (do_adjusted_abc) {
                    ...
                    candidate_activities[peak_count_helper][a_col * 3 + 1] = current_activity;
            ...
            gene_contact_scaler[chr_gene] = (max_contact != 0) ? 100 / max_contact : 0;
            // Write the relative ABC-score. And activity if not adjusting it.
            for (int r = 0; r < peak_count_helper; ++r) {
                double this_contact = candidate_contacts[r];
                for (int a_col = 0; a_col < col_num; a_col++) {
                    if (candidate_activities[r][a_col * 3 + 1] > 0) {  // W/o activity it will be 0 anyway.
                        double this_sum = abc_sums[a_col];
                        if (this_sum != 0) {
                            double this_score = (this_contact * candidate_activities[r][a_col * 3 + 1]) / this_sum;
                            if (this_score >= abc_cutoff) {
                                ...
                                if (not do_adjusted_abc) {
                                    this_hit.scaledActivity =
                                            candidate_activities[r][a_col * 3 + 1] * (100 / max_signals[a_col]);
                                }
                                else {
                                    this_hit.scaledActivity = candidate_activities[r][a_col * 3 + 1];
                                }
        ...
        for (int a_idx = 0; a_idx < col_num; a_idx++) {
            ...
            if (!exceeders[a_col].empty()) {
                ...
                auto gene_map = promoter_map[exceeders[a_col][0].gene_id];  // Get first entry, update for new genes.
                vector<double> gene_info(4, 0);
                int hit_helper = 0;
                for (auto hit : exceeders[a_col]) {
                    auto matching_peak = peak_info_map[hit.peak_id];
                    out_stream << gene_map[2] << "\t" << matching_peak.start << "\t" << matching_peak.end;
                    out_stream << "\t" << gene_map[0] << "\t" << gene_map[1];
                    ...
                    gene_info[3] += static_cast<double>(distance);
                    ...
                    if (hit_helper == exceeders[a_col].size() - 1 or exceeders[a_col][hit_helper+1].gene_id != hit.gene_id) {
                        ...
                        gene_info_stream << gene_map[0] << "\t" << gene_map[1] << "\t" << gene_map[2] << "\t"
                                         << gene_map[3];
...
#pragma omp parallel for num_threads(cores)
    for (int a_col = 0; a_col < col_num; a_col++) {
        ...
        for (const auto& entry: promoter_map) {
            ...
            if (u_genefile.length() != 0 and u_genefile != "0") {
                if ((filter_genes.count(gene_map[0]) == 0) and (filter_genes.count(gene_map[1]) == 0)) {
                    continue;
                }
            }
            string fail_message;
            if (genes_wo_hic.count(gene_id) == 1) {
                ...
                if (peak_chromosomes.count(gene_map[2]) == 0) {
                    fail_message = "No candidate regions in gene window";
                } else {
            ...
            if (!fail_message.empty()) {
                gene_info_stream << gene_map[0] << "\t" << gene_map[1] << "\t" << gene_map[2] << "\t" << gene_map[3]
