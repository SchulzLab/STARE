...
[-g input fasta file (in RefSeq format, without \"chr\" prefix_path]\n
[-j flag indicating that the reference genome contains a chr prefix_path]\n
[-p psems]\n
[-a gene annotation file, required to generate the gene view]\n
[-o prefix_path of outputfiles]\n
Optional parameters:\n
[-n column in the -b file containg the average per base signal within a peak, start counting at 1]\n
[-x bed-file with regions to exclude (e.g. blacklisted regions)]\n
[-w window size around TSS for mapping regions to genes (default 50KB/ 50MB for ABC-scoring)]\n
[-e indicating whether exponential distance decay should be used (default TRUE)]\n
...
[-z write an additional file that combines all activity columns and stacks the genes horizontally for each of them, optional input for GAZE]\n
...
while getopts "g:b:o:c:p:n:a:w:e:js:f:k:t:r:x:z" o;
do
case $o in
	g)	genome=$OPTARG;;
	b)	regions=$OPTARG;;
...
if [ -d "$prefixP" ]; then
  echo $prefixP " Folder already exists, remove or change the function call prefix in the -o flag"
  rm -r ${prefixP} # TODO CHANGE TO EXIT again
fi

mkdir ${prefixP}
prefix_path=$prefixP"/"$prefixP
...
echo -e "TF Gene Affinities\t"$prefix_path"_TF_Gene_Affinities.txt" >> $metadatafile
echo -e "Discarded genes\t"$prefix_path"_discarded_Genes.txt" >> $metadatafile
if [ -n "$hic_contactfolder" ] && [[ "$existing_abc" == "0" ]];
then
  echo -e "ABC-scoring output\t"$prefix_path"_ABCpp_scoredInteractions.txt" >> $metadatafile
fi
echo "" >> "$metadatafile"
...
# REGION PROCESSING
# ------------------------------------------------------------------------------------------------------
echo "Preprocessing region file: Removing chr prefix_path, sorting regions and removing duplicats"
...
then
  ...
  bedtools intersect -a ${filteredRegions}_sorted.bed -b ${exclude_regions}_noPrefix.bed -v > ${filteredRegions}_filtered.bed
...
${working_dir}/ReplaceInvalidChars -i "$openRegionSequences" -o ${prefix_path}_FilteredSequences.fa
...
echo "Starting TRAP"
${working_dir}/TRAPmulti $pwms ${prefix_path}_FilteredSequences.fa $cores > ${affinity}
...
then
  ...
  abc_prefix_path=${prefixP}"/ABC_output/"${prefixP}
  ${working_dir}/STARE_ABCpp_absolute -b ${filteredRegions}_sorted.bed -n ${column} -a ${annotation} -gw ${window} -cf ${hic_contactfolder} -bin ${hic_binsize} -t ${abc_cutoff} -d ${abc_prefix_path} -c ${cores}
...
${working_dir}/TF_Gene_Scorer_parallel -a ${annotation} -b ${filteredRegions}_sorted.bed -n ${column} -i ${affinity} -o ${prefixP}/Gene_TF_matrices/${prefixP} -p ${pwms} -w ${window} -e ${decay} -c ${cores} -abc ${existing_abc}
endg=`date +%s`
...
then
  ...
  ${working_dir}/Reshape_toCellTF_perGene -c ${cores} -i ${prefixP}/Gene_TF_matrices/ -o ${prefixP}/Stacked_Matrix/
fi
